...
export async function GET() {
  ...
  // Build weekly study hours (last 12 weeks)
  ...
  const lessonsOverTime = lessonProgress.map((p) => {
    lessonCount++;
    return {
      date: p.completedAt ? format(new Date(p.completedAt), "MMM d") : "",
      count: lessonCount,
    };
  });
  ...
  for (let i = 59; i >= 0; i--) {
    ...
    const dayLogs = studyLogs.filter((l) => {
      const d = new Date(l.date);
